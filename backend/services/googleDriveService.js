...
function buildSharedDriveInstruction(rootName) {
  const safeRoot = sanitizeDriveName(rootName || "AXIS PRODUCTION", "AXIS PRODUCTION");
  return (
...
async function ensureFolderPath(drive, rootFolderName, segments = []) {
  const normalizedRoot = sanitizeDriveName(rootFolderName || "AXIS PRODUCTION", "AXIS PRODUCTION");
...
async function testDriveConnection(settings = {}) {
  ...
  const folderName = sanitizeDriveName(settings.drive_root_folder_name || "AXIS PRODUCTION", "AXIS PRODUCTION");
