  ...
  await db.withDatabase(INVENTORY_DB_NAME, async () => {
    await db.query(`
      CREATE TABLE IF NOT EXISTS ${BACKUP_SETTINGS_TABLE} (
        ...
        drive_root_folder_name VARCHAR(255) NOT NULL DEFAULT 'AXIS PRODUCTION',
...
function toPublicSettings(row = {}) {
  ...
  return {
    ...
    drive_root_folder_name: sanitizeDriveName(row.drive_root_folder_name || "AXIS PRODUCTION", "AXIS PRODUCTION"),
  ...
  return db.withDatabase(INVENTORY_DB_NAME, async () => {
    ...
    const created = await db.query(
      `INSERT INTO ${BACKUP_SETTINGS_TABLE}
       ...
       VALUES ($1, FALSE, 'AXIS PRODUCTION', FALSE, FALSE, FALSE, NOW(), NOW())
       RETURNING *`,
      { bind: [targetDb] }
    );
...
async function saveBackupSettings(databaseName, body = {}, requesterId = null) {
  ...
  const driveRootFolderName = sanitizeDriveName(
    String(body.drive_root_folder_name || existing.drive_root_folder_name || "AXIS PRODUCTION").trim(),
    "AXIS PRODUCTION"
  );
...
async function backupSingleInvoiceToDrive(drive, settings, databaseName, invoiceLike) {
  ...
  const rootName = sanitizeDriveName(settings.drive_root_folder_name || "AXIS PRODUCTION", "AXIS PRODUCTION");
...
exports.testGoogleDriveConnection = async (req, res) => {
  try {
    ...
    const merged = {
      ...settingsRow,
      drive_auth_type: authType,
      drive_root_folder_name: String(req.body?.drive_root_folder_name || settingsRow.drive_root_folder_name || "AXIS PRODUCTION"),
