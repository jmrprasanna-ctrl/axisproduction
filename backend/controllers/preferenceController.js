  ...
  return db.withDatabase(targetDb, async () => {
    ...
    await db.query(
       ...
       ON CONFLICT (user_id) DO NOTHING`,
      {
        bind: [
          userId,
          String(base?.primary_color || "#4f8fb8"),
          String(base?.background_color || "#eef1f3"),
          String(base?.button_color || "#4f8fb8"),
...
exports.getMyUiSettings = async (req, res) => {
  try {
    ...
    res.json({
      ...
      primary_color: String(userPref?.primary_color || base.primary_color || "#4f8fb8"),
      accent_color: base.accent_color,
      background_color: String(userPref?.background_color || base.background_color || "#eef1f3"),
      button_color: String(userPref?.button_color || base.button_color || "#4f8fb8"),
...
exports.updateMyTheme = async (req, res) => {
  try {
    ...
    await getUserPreferenceRow(target, true);
    const primary = String(req.body?.primary_color || "").trim() || "#4f8fb8";
    const background = String(req.body?.background_color || "").trim() || "#eef1f3";
